...
export const Settings: React.FC<SettingsProps> = ({
  ...
  const saveSettings = async () => {
    try {
      ...
      // Preserve model-switcher-owned keys that may have been written out-of-band
      // (the Model tab writes settings.json directly), so saving from stale
      // General-tab state cannot silently revert the active model (review H-3).
      try {
        const onDisk = await api.getClaudeSettings();
        if (onDisk?.model) updatedSettings.model = onDisk.model;
        for (const k of ["ANTHROPIC_BASE_URL", "ANTHROPIC_MODEL"]) {
          if (onDisk?.env?.[k]) updatedSettings.env[k] = onDisk.env[k];
        }
      } catch {
        // best-effort merge; non-fatal
      }
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            <TabsList className="grid grid-cols-9 w-full mb-6 h-auto p-1">
              ...
              <TabsTrigger value="model" className="py-2.5 px-3">Model</TabsTrigger>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="space-y-1">
                        ...
                        <p className="text-caption text-muted-foreground">
                          Help improve Syndrome AI by sharing anonymous usage data
                        </p>
                      </div>
                      <Switch
