...
pub async fn save_system_prompt(content: String) -> Result<String, String> {
    ...
    fs::rename(&temp_path, &claude_md_path)
...
async fn spawn_claude_process(
    ...
    let stdout_task = tokio::spawn(async move {
        ...
        while let Ok(Some(line)) = lines.next_line().await {
                            ...
                            *session_id_guard = Some(claude_session_id.to_string());
                            ...
                            match registry_clone.register_claude_session(
                                ...
                                Ok(run_id) => {
                                    ...
                                    let mut run_id_guard = run_id_holder_clone.lock().await;
            ...
            if let Some(run_id) = *run_id_holder_clone.lock().await {
                let _ = registry_clone.append_live_output(run_id, &line);
            }

            // Emit the line to the frontend with session isolation if we have session ID
            if let Some(ref session_id) = *session_id_holder_clone.lock().await {
    ...
    let stderr_task = tokio::spawn(async move {
        ...
        while let Ok(Some(line)) = lines.next_line().await {
            ...
            if let Some(ref session_id) = *session_id_holder_clone2.lock().await {
    ...
    tokio::spawn(async move {
        ...
        if let Some(mut child) = current_process.take() {
            match child.wait().await {
                Ok(status) => {
                    ...
                    tokio::time::sleep(tokio::time::Duration::from_millis(100)).await;
                    if let Some(ref session_id) = *session_id_holder_clone3.lock().await {
        ...
        if let Some(run_id) = *run_id_holder_clone2.lock().await {
