...
impl CheckpointManager {
    ...
    pub async fn create_checkpoint(
            ...
            for entry in std::fs::read_dir(dir)? {
                ...
                if path.is_dir() {
                    if let Some(name) = path.file_name().and_then(|n| n.to_str()) {
                        ...
                        // Skip large generated directories that would cause checkpoints to hang
                        if matches!(
                            name,
                            "node_modules"
                                | "target"
        ...
        // Update current_checkpoint_id in memory and persist to disk so restarts see correct parent
