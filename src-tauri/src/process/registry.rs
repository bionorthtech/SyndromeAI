...
impl ProcessRegistry {
        ...
        let kill_result = if cfg!(target_os = "windows") {
            ...
            match &term_result {
                Ok(output) if output.status.success() => {
                    ...
                    // Give it 2 seconds to exit gracefully without blocking the tokio thread
    ...
    /// Append to live output for a process, capped at 10 MB to prevent unbounded growth
    pub fn append_live_output(&self, run_id: i64, output: &str) -> Result<(), String> {
        const MAX_LIVE_OUTPUT_BYTES: usize = 10 * 1024 * 1024; // 10 MB
        let processes = self.processes.lock().map_err(|e| e.to_string())?;
        if let Some(handle) = processes.get(&run_id) {
            let mut live_output = handle.live_output.lock().map_err(|e| e.to_string())?;
            live_output.push_str(output);
            live_output.push('\n');
            // Trim oldest content when over the cap
            if live_output.len() > MAX_LIVE_OUTPUT_BYTES {
                let trim_to = live_output.len() - MAX_LIVE_OUTPUT_BYTES;
                // Find the next newline boundary to avoid splitting a line mid-byte
                if let Some(boundary) = live_output[trim_to..].find('\n') {
                    *live_output = live_output[trim_to + boundary + 1..].to_string();
                }
            }
        }
        Ok(())
